...
public class PlaceOnTray : MonoBehaviour
{
    ...
    {
        while (transform.localPosition != Vector3.up || transform.localRotation != Quaternion.identity)
        {
            transform.localPosition = Vector3.MoveTowards(transform.localPosition, Vector3.up, Time.deltaTime * moveSpeed);
